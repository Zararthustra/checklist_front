...
function App() {
  ...
  const getTasks401 =
    useSelector((state) => state.task.error)
      ?.split(" ")
      .at(-1) === "401";
  const getCategory401 =
    useSelector((state) => state.category.error)
      ?.split(" ")
      .at(-1) === "401";
  ...
  return (
    <>
      {(getCategory401 || getTasks401) && (
        <Reconnect setTriggerToaster={setTriggerToaster} />
      )}
      <main
        style={{
          display: "flex",
      ...
      >
        <svg
          ...
          viewBox="0 0 23 26"
          fill="red"
          style={{ width: "2rem", margin: "1rem 0" }}
